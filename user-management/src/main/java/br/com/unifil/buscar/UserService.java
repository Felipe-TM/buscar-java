...
public class UserService {
	 ...
	 * it can be easily changed by modifying the {@code @Qualifier}
	 * annotation value.
	 * 
	...
	/** 
	 * Sends a request to the repository dependency to search for the given username.
	 * <p>
	...
	public User loadByUsername(String username) throws UsernameNotFoundException {
		...
		return repository.loadByUsername(username).orElseThrow(() -> new UsernameNotFoundException("User with username " + username +  " not found"));
	}

	/**
	 * Sends a request to the repository dependency to save the new user to
	 * the database.
	 * <p>
	...
	public void save(UserRecord userRecord) {
		repository.save(new User.Builder()
				.name(userRecord.name())
				.phoneNumber(userRecord.phoneNumber())
				.username(userRecord.username())
				.password(userRecord.passowrd())
				.build());
