...
public class RideController {
    ...
    @PostMapping("api/v1/start-ride")
    public ResponseEntity<String> startRide(@RequestBody String requestBody) {
	JsonObject jsonRequest = JsonParser.parseString(requestBody).getAsJsonObject();
	
	try {
	    rideService.startRide(jsonRequest.get("rideId").getAsString(), jsonRequest.get("driverId").getAsString());
    ...
    @PostMapping("api/v1/end-ride")
    public ResponseEntity<String> endRide(@RequestBody String requestBody) {
	JsonObject jsonRequest = JsonParser.parseString(requestBody).getAsJsonObject();
	
	try {
	    rideService.endRide(jsonRequest.get("rideId").getAsString(), jsonRequest.get("driverId").getAsString());
	    return ResponseEntity.status(HttpStatus.OK).body("");
	} catch (NullPointerException | IllegalArgumentException e) {
	    System.out.println(e.getMessage());
	    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
